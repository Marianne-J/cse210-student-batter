...
class HandleCollisionsAction(Action):
    ...
    def execute(self, cast):
        ...
        if (ball_next_x <= 0 or ball_next_x >= constants.MAX_X):
            ball.set_velocity(ball_vel.reverse_x())
        # ceiling collision
        elif (ball_next_y <= 0):
            ball.set_velocity(ball_vel.reverse_y())
        # floor collision, ends the game if true
        elif (ball_pos.get_y() > constants.MAX_Y):
            sys.exit()
        # paddle collision, checks vertically
        elif ( ball_next_y == paddle_pos.get_y() ) and ( ball_pos.get_x() >= paddle_pos.get_x() and ball_pos.get_x() <= paddle_pos.get_x() + 10):
            ball.set_velocity(ball_vel.reverse_y())
        # brick collision
        else:
            for brick in bricks:
